...
- 대시보드의 **로그 전체 복사**로 실행 기록 전체를 클립보드에 담거나 **로그 폴더 열기**로 Finder에서 기록 위치를 바로 확인할 수 있습니다.
- 밀리의서재가 `1 / 449` 쪽수를 `1`, `/`, `449`처럼 분리된 접근성 요소로 제공하는 책도 자동으로 인식합니다.
...
### 보안 권한을 유지하는 코드 전용 업데이트

문제를 수정한 새 버전이 올라왔을 때는 아래 명령 한 줄을 터미널에 붙여넣으세요.

```bash
/bin/zsh -c "$(/usr/bin/curl -fsSL https://raw.githubusercontent.com/groundroot/MyBookOCR/main/update_code_only.sh)"
```

이 명령은 GitHub의 최신 실행 스크립트, OCR 처리기와 대시보드만 교체합니다. 기존 `마이북.app` 번들을 열거나 다시 만들거나 재서명하지 않으며, 손쉬운 사용·화면 녹화·자동화 권한 설정도 실행하지 않습니다. OCR 모델, 결과 파일, 이어서 작업 정보와 사용자 설정도 그대로 유지합니다.

작업 중 파일이 섞이지 않도록 캡처 또는 OCR이 실행 중이면 업데이트하지 않고 안내 후 중단합니다. 앱이 아직 설치되지 않은 Mac에서는 코드 전용 업데이트 대신 [한 줄 설치](#한-줄-설치)를 먼저 실행해야 합니다.
...
> [!IMPORTANT]
> 밀리의서재 뷰어는 반드시 **가로 보기**로 설정하고 한 쪽씩 보이게 해야 합니다. 스크롤 모드에서는 페이지 번호를 안전하게 확인할 수 없으므로 마이북이 캡처를 시작하지 않습니다.
...
- 다른 밀리의서재 버전에서 쪽수가 접근성 이름 대신 설명·값·일반 슬라이더로 제공되는 경우도 자동으로 탐지합니다. `현재/전체`, `current of total`, `전체 페이지 중 현재 페이지` 형식을 지원합니다.
- 그래도 쪽수를 찾지 못하면 `~/.cache/millie-ocr/reader-state.txt`에 해당 Mac의 접근성 구조를 자동 저장합니다. 문제를 신고할 때 이 파일을 함께 보내면 됩니다.
- **밀리의서재 뷰어는 반드시 `가로 보기`로 설정해야 합니다. 스크롤 모드는 지원하지 않습니다.** 쪽수를 찾지 못하면 잘못된 이미지를 저장하지 않도록 캡처를 시작하지 않고 대시보드에 설정 방법을 표시합니다.
- 목차, 설정, 완료 팝업을 닫습니다.
...
로컬 대시보드의 **결과 열기**에서 **로그 전체 복사**를 누르면 위 파일의 전체 텍스트가 클립보드에 복사됩니다. **로그 폴더 열기**를 누르면 Finder에서 기록 폴더가 열립니다. 로그에는 오류 진단 정보가 포함될 수 있으므로 공유하기 전에 개인 경로 등 민감한 내용이 없는지 확인하세요. 외부 조회 화면에서는 두 기능이 표시되지 않으며 서버에서도 접근이 차단됩니다.
