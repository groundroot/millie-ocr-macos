...
- 접근성 구조에 쪽수 자체가 없는 EPUB 뷰어는 본문 영역의 화면 지문을 비교하는 무카운터 모드로 자동 전환합니다. 왼쪽 키로 본문이 더 이상 바뀌지 않는 지점을 첫 화면으로 확정하고, 오른쪽 키 입력 뒤 본문이 실제로 달라질 때만 다음 이미지 번호를 부여합니다. 전체 화면 수는 마지막 장을 확인한 뒤 확정되므로 캡처 중에는 활동 막대와 저장된 화면 수를 표시합니다.
- 목차, 설정, 완료 팝업을 닫습니다.
